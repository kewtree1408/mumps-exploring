...
RUN apt-get -y install autoconf libreadline6 libpcre3 libpcre3-dev g++ wget libgmp-dev libgmpada5-dev libreadline-dev make sudo libelf-dev vim
...
RUN chmod +x gtminstall
RUN sudo ./gtminstall --utf8 default
RUN echo "source /usr/lib/fis-gtm/V6.3-001A_x86_64/gtmprofile" >> /root/.bashrc
CMD [ "sleep", "365d" ]
